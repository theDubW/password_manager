...
kvs = {"domain1": "password1", "domain2": "password2", "domain3": "password3"}
...
for domain in kvs:
    ...
    if pw != kvs[domain]:
        error(
            "get failed for domain %s (expected %s, received %s)"
            % (domain, kvs[domain], pw)
        )
    else:
...
for domain in kvs:
    ...
    if pw1 != pw2:
        error(
            "get mismatch for domain %s (received values %s and %s)"
            % (domain, pw1, pw2)
        )
